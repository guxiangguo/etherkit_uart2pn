...
/* Network endianness */
uint8_t _inputdata[APP_GSDML_INTPUT1_DATA_SIZE] = {0};
uint8_t _outputdata[APP_GSDML_OUTPUT1_DATA_SIZE] = {0};
   ...
   uint8_t * iops)
{
   ...
   if (submodule_id == APP_GSDML_SUBMOD_ID_INPUT1)
   {
      //_inputdata[0] = counter++;
      *size = APP_GSDML_INTPUT1_DATA_SIZE;
      *iops = PNET_IOXS_GOOD;      
      return _inputdata;
   }
...
int app_data_set_output_data (
   ...
   uint16_t size)
{
   static bool led_state = RT_FALSE;
      ...
      submodule_id == APP_GSDML_SUBMOD_ID_DIGITAL_IN_OUT)
   {
      if (size == APP_GSDML_OUTPUT_DATA_DIGITAL_SIZE)
      {
         ...
         if (0 != rt_memcmp(outputdata, data, size))
         {
   ...
   else if (submodule_id == APP_GSDML_SUBMOD_ID_OUTPUT1)
   {
      if (size == APP_GSDML_OUTPUT1_DATA_SIZE)
      {
         if (0 != rt_memcmp(_outputdata, data, size))
         {
            led_state = !led_state;
            app_handle_data_led_state (led_state);    
         }         
         memcpy (_outputdata, data, size);
         return 0;
      }
   }
